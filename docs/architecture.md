...
- **Composable:** frontend, gateway, ML service, vector store, and LLM provider are independent and replaceable.
- **Clean service boundary:** business state lives in the C# gateway; ML compute lives in the Python service. Neither knows about the other's domain.
...
| API Gateway (C#) | Users, roles, JWT issuance, RBAC, tenant scoping, document metadata (EF Core), conversations, messages, audit logs, rate limiting, orchestration of ML calls | Embedding math, vector search, LLM calls         |
...
1. **Frontend → Gateway.** Next.js sends an authenticated request (`POST /chat`) to the ASP.NET Core gateway with the user's question and `document_set_id`.
2. **Gateway validates.** Gateway validates JWT, checks tenant / RBAC, writes an audit log entry, persists the user's message to `messages`.
3. **Gateway → ML.** Gateway calls the Python ML service (`POST /answer`) over REST with `{ question, document_set_id, conversation_context }`.
...
5. **Gateway persists.** Gateway writes the assistant response to `messages` (including `sources` JSON, latency, token usage) and updates the conversation.
6. **Frontend renders.** Gateway streams the response back to the frontend, which renders the answer with clickable source citations.
...
- **gRPC** — for tighter contracts and lower per-call overhead on the gateway → ML hop.
- **OpenAPI codegen** — generate typed clients in both C# and Python from a single spec; keeps HTTP/JSON but enforces the schema.

**Error format.** ML service returns RFC 7807 problem details (`application/problem+json`). Gateway translates 5xx into a single user-facing error and surfaces the correlation ID for tracing.

## Key Principle

> The C# gateway is the **system of record** for all user, auth, and business state.
...
- The C# gateway never embeds, never queries `chunks.embedding`, never calls Ollama directly.
...
|-----------------------------------|-------------------------------------------|-------------------------------------------------------------------------------------------------|
| Split into two backend languages  | C# for gateway, Python for ML             | Each language is dominant in its lane; demonstrates polyglot service design                     |
...
| Gateway = system of record        | All business state in C#                  | Clean ownership boundary; Python service stays stateless and horizontally scalable              |
